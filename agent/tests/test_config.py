...
def test_settings_accepts_valid_urls_and_redacts_key() -> None:
    settings = Settings(
        ...
        OPENROUTER_API_KEY="voice-secret",
    ...
    assert settings.openrouter_api_key_value == "voice-secret"
    assert settings.safe_summary()["openrouter_api_key"] == "***"
    assert settings.openrouter_transcribe_model == "openai/gpt-4o-transcribe"
...
def test_missing_key_is_reported_cleanly() -> None:
    settings = Settings(DEEPSEEK_API_KEY=None, OPENROUTER_API_KEY=None)
    ...
    with pytest.raises(RuntimeError, match="OPENROUTER_API_KEY"):
        _ = settings.openrouter_api_key_value
